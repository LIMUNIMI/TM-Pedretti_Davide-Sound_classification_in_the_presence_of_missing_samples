...
class ContextEncoder():
    ...
    def save_images(self, imgs, path):
       ...
       for i in tqdm(range(len(imgs))): #range(len(imgs))
          ...
          next_frames[i] = next_frames[i].reshape(1,self.mask_width,128,1)
          gen_missing = self.generator([previous_frames[i], next_frames[i]])
          #print(gen_missing.numpy().reshape(self.mask_width,128).T.shape)
          masked_imgs[i][: , x1_mel[i]:x2_mel[i]] = gen_missing.numpy().reshape(self.mask_width,128).T
          waveform = librosa.feature.inverse.mel_to_audio(masked_imgs[i], sr=self.sr, hop_length=501)
          padded_waveform = np.pad(waveform,(0, 32000-waveform.shape[0]), mode='constant')
          reconstructed_audio.append(padded_waveform)       
    ...
    def save_model(self):
        def save(model, model_name):
            # path = '/content/gdrive/MyDrive/models/generator'
            model_path = "saved_model/%s" % model_name # es. saved_model/generator
...
if __name__ == '__main__':
    context_encoder = ContextEncoder(corr_length=1000)
    context_encoder.train(epochs=60, batch_size=128, sample_interval=10) # it was 20, 64
    context_encoder.save_images(context_encoder.X_train, "./UrbanSound8K/dati/full_reconstructed_8kHz_%s.npz" % context_encoder.corruption_length)
